...
/**
 * Handles high-level decoding and seeking in FLAC files. Also returns metadata blocks. Every object is stateful, not thread-safe, and needs to be closed.
 * Sample usage:
 *
 * <pre>
 * // Create a decoder
...
public final class FlacDecoder implements AutoCloseable {
    ...
    /**
     * Reads, handles, and returns the next metadata block. Returns a pair (Integer type, byte[] data) if the next metadata block exists, otherwise returns null
     * if the final metadata block was previously read. In addition to reading and returning data, this method also updates the internal state of this object to
     * reflect the new data seen, and throws exceptions for situations such as not starting with a stream info metadata block or encountering duplicates of
     * certain blocks.
     *
     * @return pair of (int type, byte[] data)
     *
     * @throws IOException
     */
    ...
    /**
     * Reads and decodes the next block of audio samples into the given buffer, returning the number of samples in the block. The return value is 0 if the read
     * started at the end of stream, or a number in the range [1, 65536] for a valid block. All metadata blocks must be read before starting to read audio
     * blocks.
     *
     * @param samples
     * @param off
     *
     * @return number of samples in the block
     *
     * @throws IOException
     */
    ...
    /**
     * Seeks to the given sample position and reads audio samples into the given buffer, returning the number of samples filled. If audio data is available then
     * the return value is at least 1; otherwise 0 is returned to indicate the end of stream. Note that the sample position can land in the middle of a FLAC
     * block and will still behave correctly. In theory this method subsumes the functionality of readAudioBlock(), but seeking can be an expensive operation so
     * readAudioBlock() should be used for ordinary contiguous streaming.
     *
     * @param pos
     * @param samples
     * @param off
    ...
    /**
     * Returns a pair (sample offset, file position) such sampleOffset <= pos and abs(sampleOffset - pos) is a relatively small number compared to the total
     * number of samples in the audio file. This method works by skipping to arbitrary places in the file, finding a sync sequence, decoding the frame header,
     * examining the audio position stored in the frame, and possibly deciding to skip to other places and retrying. This changes the state of the input streams
     * as a side effect. There is a small chance of finding a valid-looking frame header but causing erroneous decoding later.
     *
     * @param pos
     *
     * @return
    ...
    private long[] seekBySyncAndDecode(long pos) throws IOException {
        ...
        while (end - start > 100000) { // Binary search
            ...
            final long[] offsets = this.getNextFrameOffsets(mid);
    ...
    /**
     * Returns a pair (sample offset, file position) describing the next frame found starting at the given file offset, or null if no frame is found before the
     * end of stream. This changes the state of the input streams as a side effect.
     *
     * @param filePos
     *
     * @return
    ...
    /**
     * Closes the underlying input streams and discards object data. This decoder object becomes invalid for any method calls or field usages.
     */
    @Override
