...
/**
 * Represents most fields in a frame header, in decoded (not raw) form. Mutable structure, not thread safe. Also has methods for parsing and serializing this
...
public final class FrameInfo {
    ...
    /**
     * The index of this frame, where the foremost frame has index 0 and each subsequent frame increments it. This is either a uint31 value or &minus;1 if
     * unused. Exactly one of the fields frameIndex and sampleOffse is equal to &minus;1 (not both nor neither). This value can only be used if the stream
     * info's minBlockSize = maxBlockSize (constant block size encoding style).
     */
    public int frameIndex;

    /**
     * The offset of the first sample in this frame with respect to the beginning of the audio stream. This is either a uint36 value or &minus;1 if unused.
     * Exactly one of the fields frameIndex and sampleOffse is equal to &minus;1 (not both nor neither).
     */
    public long sampleOffset;

    /**
     * The number of audio channels in this frame, in the range 1 to 8 inclusive. This value is fully determined by the channelAssignment field.
     */
    public int numChannels;

    /**
     * The raw channel assignment value of this frame, which is a uint4 value. This indicates the number of channels, but also tells the stereo coding mode.
     */
    ...
    /**
     * The sample rate of this frame in hertz (Hz), in the range 1 to 655360 inclusive, or &minus;1 if unavailable (i.e. the stream info should be consulted).
     */
    public int sampleRate;

    /**
     * The sample depth of this frame in bits, in the range 8 to 24 inclusive, or &minus;1 if unavailable (i.e. the stream info should be consulted).
     */
    public int sampleDepth;

    /**
     * The size of this frame in bytes, from the start of the sync sequence to the end of the trailing CRC-16 checksum. A valid value is at least 10, or
     * &minus;1 if unavailable (e.g. the frame header was parsed but not the entire frame).
     */
    ...
    /**
     * Reads the next FLAC frame header from the specified input stream, either returning a new frame info object or {@code null}. The stream must be aligned to
     * a byte boundary and start at a sync sequence. If EOF is immediately encountered before any bytes were read, then this returns {@code null}.
     * <p>
     * Otherwise this reads between 6 to 16 bytes from the stream &ndash; starting from the sync code, and ending after the CRC-8 value is read (but before
     * reading any subframes). It tries to parse the frame header data. After the values are successfully decoded, a new frame info object is created, almost
     * all fields are set to the parsed values, and it is returned. (This doesn't read to the end of the frame, so the frameSize field is set to -1.)
     * </p>
     *
    ...
    public static FrameInfo readFrame(FlacLowLevelInput in) throws IOException {
        ...
        final long position = FrameInfo.readUtf8Integer(in); // Reads 1 to 7 bytes
        if (blockStrategy == 0) {
            if (position >>> 31 != 0) {
    ...
    /**
     * Reads 1 to 7 whole bytes from the input stream. Return value is a uint36. See: https://hydrogenaud.io/index.php/topic,112831.msg929128.html#msg929128
     *
     * @param in
     *
     * @return
    ...
    /**
     * Argument is a uint4 value. Reads 0 to 2 bytes from the input stream. Return value is in the range [1, 65536].
     *
     * @param code
     * @param in
     *
    ...
    private static int decodeBlockSize(int code, FlacLowLevelInput in) throws IOException {
        ...
        switch (code) {
            ...
            default:
                final int result = FrameInfo.searchSecond(FrameInfo.BLOCK_SIZE_CODES, code);
    ...
    /**
     * Argument is a uint4 value. Reads 0 to 2 bytes from the input stream. Return value is in the range [-1, 655350].
     *
     * @param code
     * @param in
     *
    ...
    private static int decodeSampleRate(int code, FlacLowLevelInput in) throws IOException {
        ...
        switch (code) {
            ...
            default:
                final int result = FrameInfo.searchSecond(FrameInfo.SAMPLE_RATE_CODES, code);
    ...
    private static int decodeSampleDepth(int code) {
        ...
        final int result = FrameInfo.searchSecond(FrameInfo.SAMPLE_DEPTH_CODES, code);
        if (result == -1) {
    ...
    /**
     * Writes the current state of this object as a frame header to the specified output stream, from the sync field through to the CRC-8 field (inclusive).
     * This does not write the data of subframes, the bit padding, nor the CRC-16 field.
     * </p>
     * <p>
     * The stream must be byte-aligned before this method is called, and will be aligned upon returning (i.e. it writes a whole number of bytes). This method
     * initially resets the stream's CRC computations, which is useful behavior for the caller because it will need to write the CRC-16 at the end of the frame.
     * </p>
     *
    ...
    public void writeHeader(BitOutputStream out) throws IOException {
        ...
        final int blockSizeCode = FrameInfo.getBlockSizeCode(this.blockSize);
        out.writeInt(4, blockSizeCode);
        final int sampleRateCode = FrameInfo.getSampleRateCode(this.sampleRate);
        ...
        if (this.frameIndex != -1 && this.sampleOffset == -1) {
            FrameInfo.writeUtf8Integer(this.sampleOffset, out);
        } else if (this.sampleOffset != -1 && this.frameIndex == -1) {
            FrameInfo.writeUtf8Integer(this.sampleOffset, out);
        } else {
    ...
    private static int getBlockSizeCode(int blockSize) {
        int result = FrameInfo.searchFirst(FrameInfo.BLOCK_SIZE_CODES, blockSize);
        if (result != -1) {
            // Already done
    ...
    private static int getSampleRateCode(int sampleRate) {
        ...
        int result = FrameInfo.searchFirst(FrameInfo.SAMPLE_RATE_CODES, sampleRate);
        if (result != -1) {
            // Already done
    ...
    private static int getSampleDepthCode(int sampleDepth) {
        ...
        int result = FrameInfo.searchFirst(FrameInfo.SAMPLE_DEPTH_CODES, sampleDepth);
        if (result == -1) {
            result = 0;
        }
        if (result >>> 3 != 0) {
    ...
    private static final int[][] BLOCK_SIZE_CODES = { { 192, 1 }, { 576, 2 }, { 1152, 3 }, { 2304, 4 }, { 4608, 5 }, { 256, 8 }, { 512, 9 }, { 1024, 10 },
            { 2048, 11 }, { 4096, 12 }, { 8192, 13 }, { 16384, 14 }, { 32768, 15 }, };

    private static final int[][] SAMPLE_DEPTH_CODES = { { 8, 1 }, { 12, 2 }, { 16, 4 }, { 20, 5 }, { 24, 6 }, };

    private static final int[][] SAMPLE_RATE_CODES = { { 88200, 1 }, { 176400, 2 }, { 192000, 3 }, { 8000, 4 }, { 16000, 5 }, { 22050, 6 }, { 24000, 7 },
